"""Export-only PyTorch Laya DecisionModel (source of weights for the Core ML graph).

Vendored from mizorewww/laya-coreml@4619e0483f07adf39068532e85b42ec2347edb83,
laya_coreml/torch_model.py (Apache-2.0; see NOTICE). Only this docstring differs.
Explicit attention/head modules avoid Transformers' tracing-time backend switches;
state-dict names match the original Laya checkpoints.
"""
...
class Embeddings(nn.Module):
    def __init__(self, cfg):
        ...
        self.norm = nn.LayerNorm(cfg["hidden_size"], eps=cfg.get("norm_eps", 1e-5), bias=cfg.get("norm_bias", False))
...
class Encoder(nn.Module):
    def __init__(self, cfg, max_length):
        ...
        kinds = cfg.get("layer_types") or [
            "full_attention" if i % cfg.get("global_attn_every_n_layers", 3) == 0 else "sliding_attention"
...
class HeadAttention(nn.Module):
    ...
    def forward(self, x, mask):
        b, n, _ = x.shape
        qkv = F.linear(x, self.in_proj_weight, self.in_proj_bias).reshape(b, n, 3, self.heads, self.dim)
        q, k, v = (qkv[:, :, i].transpose(1, 2) for i in range(3))
